...
// Last-resort label for a key missing from every catalog: "nav.overview"
// reads as "Overview", never as a dotted key path. A missing translation is
// a bug, but it should look like a rough label to the user rather than
// leaking internals into the page.
function humanize(key) {
  const leaf = key.split('.').pop().replace(/[_-]+/g, ' ').trim();
  if (!leaf) return '';
...
function translate(lang, key, vars) {
  ...
  if (!warnedMissingKeys.has(key)) {
    ...
    console.warn(`[i18n] missing translation for "${key}" - falling back to a humanized label`);
  }
